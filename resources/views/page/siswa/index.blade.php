                                ...
                                @foreach ($datas as $data)
                                    <tr
                                        ...
                                        <td class="px-6 py-4 flex">
                                            ...
                                            <form action="{{ url("/siswa/delete/$data->id") }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    onclick="return confirm('Apakah Anda yakin ingin menghapus item ini?')"
                                                    class="text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-xm px-2 py-1 text-center me-2 mb-2">Delete</button>
                                            </form>
                                        </td>
                                    <tr>
                                @endforeach
